...
import { GraduationCap, ArrowLeft, Mail, Lock, Eye, EyeOff, KeyRound, CheckCircle2 } from 'lucide-react';
...
export default function StudentForgotPassword() {
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/send-otp', { email, role: 'student' });
      setStep(2);
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/verify-otp', { email, otp, role: 'student' });
      setStep(3);
    } catch (err) {
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/auth/reset-password', { email, otp, newPassword, role: 'student' });
      setSuccess(true);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="w-full max-w-md text-center animate-fade-in-up">
          <div className="card p-8">
            ...
            <Link to="/student/login" className="btn-primary">Go to Login</Link>
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-md animate-fade-in-up">
        ...
        <div className="card p-8">
          ...
          <div className="mt-6 text-center space-y-2">
            ...
            <Link to="/student/login" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600 transition-colors">
